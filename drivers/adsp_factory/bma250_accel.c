...
static int __devinit bma250accel_factory_init(void)
{
	adsp_factory_register("accelerometer_sensor",ADSP_FACTORY_MODE_ACCEL,acc_attrs,&adsp_fact_cb);
	pr_err("(%s): factory \n", __func__);
	return 0;
}
static void __devexit bma250accel_factory_exit(void)
{
	pr_err("(%s): factory \n", __func__);
}
module_init(bma250accel_factory_init);
module_exit(bma250accel_factory_exit);
